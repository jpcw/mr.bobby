...
class parse_configTest(unittest.TestCase):
    ...
    def call_FUT(self, configname='example.ini'):
        ...
        if not configname.startswith('http'):
            f = os.path.abspath(
                os.path.join(
                    os.path.dirname(mrbobby.__file__), 'tests', configname)
            )
        else:
            f = configname
    ...
    def test_parse_variable(self):
        c = self.call_FUT()
        self.assertEqual(c['variables']['name'], 'Bobby')
    ...
    def test_ignored(self):
        ...
        self.assertTrue('ignored' in c['mr.bobby']['ignored_files'])
        self.assertTrue('*.txt' in c['mr.bobby']['ignored_files'])
    ...
    def test_parse_config(self):
        ...
        expected_output = [
            ...
            'webserver.fqdn = mrbobby.10.0.10.120.xip.io',
    ...
    @mock.patch('mrbobby.parsing.urlretrieve')
